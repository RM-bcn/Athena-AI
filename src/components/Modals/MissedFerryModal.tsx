...
import { X, Ship, CheckCircle, MapPin, MessageSquare } from 'lucide-react';
import { FerryResolution } from '../../types';

interface MissedFerryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRequestHelp: (optionName: string) => void;
}
...
export const MissedFerryModal: React.FC<MissedFerryModalProps> = ({
  ...
  const [resolution] = useState<FerryResolution>({
    ...
    advice: 'Deze vertrektijden zijn voorbeelden. Laat Athena via de chat je actuele alternatieven opzoeken.'
  });

  if (!isOpen) return null;

  const handleRequestHelp = (optionName: string) => {
    setPassingToChat(true);
    onRequestHelp(optionName);
  };
        ...
        <div className="space-y-6">
          <div className="p-4 bg-[#f0f6ff] rounded-2xl border border-[#005BAE]/20">
            <p className="font-['Inter'] text-xs text-[#002a52] leading-relaxed">
              <strong>Athena:</strong> Deze vertrektijden zijn voorbeelden. Laat Athena via de chat
              je actuele alternatieven opzoeken.
            </p>
          </div>
            ...
            <div className="space-y-3">
              {resolution.options.map((opt, idx) => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleRequestHelp(opt.operator)}
